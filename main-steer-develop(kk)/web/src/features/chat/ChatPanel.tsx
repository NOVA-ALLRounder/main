...
function InlineRoutineCard({ hint, onClose, onCreated, onError }: { hint: string; onClose: () => void; onCreated: () => void; onError: (msg: string) => void }) {
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            onError(`猷⑦떞 ?앹꽦 ?ㅽ뙣: ${e instanceof Error ? e.message : "?????녿뒗 ?ㅻ쪟"}`);
        } finally {
            setCreating(false);
        }
    };
...
export default function ChatPanel() {
    ...
    const [messages, setMessages] = useState<Message[]>([
        { role: "assistant", text: "Hello! I am Steer OS. Try saying '留ㅼ씪 ?꾩묠 ?댁뒪 ?붿빟 ?댁쨾' to create a routine!" }
    ]);
    ...
    const mutation = useMutation({
        ...
        onSuccess: (data) => {
            setMessages(prev => [...prev, { role: "assistant", text: data.response, command: data.command, errorCode: data.error_code }]);
        ...
        onError: (error) => {
            ...
            setMessages(prev => [...prev, { role: "assistant", text: `Failed to reach core agent: ${detail}` }]);
        }
    });
    ...
    const handleSend = () => {
        ...
        if (routineIntent) {
            ...
            setMessages(prev => [...prev, {
                role: "assistant",
                text: "?뱥 Got it! I've prepared a routine creation form for you. Fill in the details below:",
                ...
                <div className="p-4 bg-white/5 border-t border-white/5">
                    <div className="flex gap-2 relative">
                        <input
                            ...
                            placeholder="Try: '留ㅼ씪 ?꾩묠 ?댁뒪 ?붿빟 ?댁쨾'"
