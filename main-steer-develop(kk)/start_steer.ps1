    ...
    while ((Get-Date) -lt $deadline) {
        ...
        foreach ($owner in $owners) {
            try {
                ...
                Write-Host "Stopped stale listener on port $Port (pid: $owner)"
            } catch {
                Write-Warning "Could not stop listener pid $owner on port ${Port}. $($_.Exception.Message)"
            }
        }
...
if ($RunOnly) {
    ...
    if (-not (Ensure-PortAvailable -Port 5680 -TimeoutSeconds 10)) {
        throw "RunOnly blocked: port 5680 is still occupied by another process. Close it (or reboot) and retry."
    }
    ...
    if (Ensure-CoreApi -ReleaseDir $releaseDir -TimeoutSeconds 60) {
        ...
        Stop-SteeerProcesses
        throw "RunOnly failed: API health check did not pass. Prevented blank-screen session."
    }
    exit 0
}
...
if ($Dev) {
    ...
    if (-not (Ensure-PortAvailable -Port 5680 -TimeoutSeconds 10)) {
        Pop-Location
        throw "Dev launch blocked: port 5680 is still occupied by another process."
    }
    npm run tauri dev
    Pop-Location
    exit 0
...
if (-not (Ensure-PortAvailable -Port 5680 -TimeoutSeconds 10)) {
    Pop-Location
    throw "Launch blocked: port 5680 is still occupied by another process."
}
...
if (Ensure-CoreApi -ReleaseDir $releaseDir -TimeoutSeconds 60) {
    ...
    Stop-SteeerProcesses
    throw "Launch failed: API health check did not pass. Prevented blank-screen session."
}
