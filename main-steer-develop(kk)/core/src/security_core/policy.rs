    ...
    pub fn check_with_context(
        ...
        match level {
            ...
            SecurityLevel::Critical => {
                if self.write_lock {
                    Err("Write Lock Engaged: Critical action blocked until unlocked.".to_string())
                } else if critical_action_approved(action, cwd) {
                    Ok(())
                } else {
                    Err(
                        "Critical action blocked. Set STEER_ENABLE_CRITICAL_ACTIONS=1 and STEER_CRITICAL_CONFIRM=I_UNDERSTAND_CRITICAL_ACTIONS to proceed."
                            .to_string(),
                    )
                }
            }
...
fn critical_action_approved(action: &AgentAction, cwd: Option<&str>) -> bool {
    if !env_bool("STEER_ENABLE_CRITICAL_ACTIONS", false) {
        return false;
    }
    if env::var("STEER_CRITICAL_CONFIRM")
        .ok()
        .map(|v| v.trim().to_string())
        .unwrap_or_default()
        != "I_UNDERSTAND_CRITICAL_ACTIONS"
    {
        return false;
    }
    ...
    match action {
        AgentAction::ShellExecution { command } => {
            env_bool("STEER_ALLOW_CRITICAL_SHELL", false) && is_shell_command_allowed(command, cwd)
        }
        AgentAction::Terminate => env_bool("STEER_ALLOW_TERMINATE", false),
        _ => true,
    }
}
...
mod tests {
    ...
    #[test]
    fn test_critical_terminate_requires_explicit_confirmation() {
        std::env::remove_var("STEER_ENABLE_CRITICAL_ACTIONS");
        std::env::remove_var("STEER_CRITICAL_CONFIRM");
        std::env::remove_var("STEER_ALLOW_TERMINATE");

        let mut policy = PolicyEngine::new();
        policy.unlock();
        let action = AgentAction::Terminate;
        assert!(policy.check(&action).is_err());

        std::env::set_var("STEER_ENABLE_CRITICAL_ACTIONS", "1");
        std::env::set_var("STEER_CRITICAL_CONFIRM", "I_UNDERSTAND_CRITICAL_ACTIONS");
        std::env::set_var("STEER_ALLOW_TERMINATE", "1");
        assert!(policy.check(&action).is_ok());
