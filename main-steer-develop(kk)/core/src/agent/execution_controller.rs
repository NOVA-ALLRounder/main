...
pub async fn execute_plan(plan: &Plan, start_index: usize) -> ExecutionResult {
    ...
    for (idx, step) in plan.steps.iter().enumerate().skip(start_index) {
        ...
        match step.step_type {
            ...
            StepType::Screenshot => {}
            StepType::Type => {
                if let Some(text) = step.data.get("text").and_then(|v| v.as_str()) {
                    #[cfg(target_os = "macos")]
                    if let Err(e) = crate::applescript::type_text(text) {
                        logs.push(format!("Mac type failed: {}", e));
                        manual_required = true; 
                    }
                    #[cfg(target_os = "windows")]
                    if let Err(e) = crate::windows::actions::type_text(text) {
                        logs.push(format!("Windows type failed: {}", e));
                        manual_required = true;
                    }
                    logs.push(format!("Typed '{}'", text));
                } else {
                    logs.push("Type step missing text".to_string());
                }
            }
            StepType::Shell => {
                if let Some(cmd) = step.data.get("command").and_then(|v| v.as_str()) {
                    match approval_gate::ApprovalGate::check_command(cmd) {
                        approval_gate::ApprovalLevel::Blocked => {
                            blocked = true;
                            approval_context = Some(ApprovalContext {
                                action: cmd.to_string(),
                                message: "Blocked by approval policy".to_string(),
                                risk_level: "critical".to_string(),
                                policy: "shell_safety".to_string(),
                            });
                            logs.push(format!("Shell blocked by policy: {}", cmd));
                        }
                        approval_gate::ApprovalLevel::RequireApproval => {
                            approval_required = true;
                            approval_context = Some(ApprovalContext {
                                action: cmd.to_string(),
                            ...
                            logs.push(format!("Shell approval required: {}", cmd));
                        }
                        approval_gate::ApprovalLevel::AutoApprove => {
                            ...
                            match output {
                                Ok(o) => {
                                    ...
                                    logs.push(format!("Shell: {}", cmd));
                                    if !stdout.is_empty() {
                                        logs.push(format!("Out: {}", stdout.trim()));
                                    }
                                    if !stderr.is_empty() {
                                        logs.push(format!("Err: {}", stderr.trim()));
                                    }
                                }
                                Err(e) => logs.push(format!("Shell failed: {}", e)),
                            }
                        }
                    }
                }
            ...
            StepType::OpenApp => {
                if let Some(app) = step.data.get("name").and_then(|v| v.as_str()) {
                    logs.push(format!("Opening app: {}", app));
                    #[cfg(target_os = "macos")]
                    let _ = crate::applescript::launch_app(app);
                    #[cfg(target_os = "windows")]
                    let _ = crate::windows::actions::launch_app(app);
                }
            }
            StepType::Shortcut => {
                if let Some(keys) = step.data.get("keys").and_then(|v| v.as_str()) {
                    logs.push(format!("Shortcut: {}", keys));
                    #[cfg(target_os = "macos")]
                    let _ = crate::applescript::keystroke(keys); 
                    // Note: applescript module needs keystroke fn if not present
                    #[cfg(target_os = "windows")]
                    let _ = crate::windows::actions::send_keys(keys);
                }
            }
        }
