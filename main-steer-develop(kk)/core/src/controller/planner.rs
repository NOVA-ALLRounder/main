...
use crate::controller::planner_helpers;
...
impl Planner {
    fn scenario_mode_enabled() -> bool {
        planner_helpers::scenario_mode_enabled()
    }

    fn history_contains_case_insensitive(history: &[String], needle: &str) -> bool {
        planner_helpers::history_contains_case_insensitive(history, needle)
    }

    fn last_opened_app_from_history(history: &[String]) -> Option<String> {
        planner_helpers::last_opened_app_from_history(history)
    }

    fn goal_contains_any(goal_lower: &str, needles: &[&str]) -> bool {
        planner_helpers::goal_contains_any(goal_lower, needles)
    }

    fn is_textual_app(app: &str) -> bool {
        planner_helpers::is_textual_app(app)
    }
