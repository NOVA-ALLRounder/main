...
impl CrossAppBridge {
    ...
    fn run_osascript_output(script: &str, timeout_ms: u64) -> Result<Output> {
        ...
        {
            Err(anyhow::anyhow!("run_osascript_output not supported on this OS"))
        }
    }
    ...
    pub fn get_clipboard() -> Result<String> {
        ...
        #[cfg(not(any(target_os = "macos", target_os = "windows")))]
        {
            Ok("Clipboard not supported".to_string())
        }
    }
    ...
    pub fn switch_to_app(app_name: &str) -> Result<()> {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if !status.success() {
                 // return Err(anyhow::anyhow!("Failed to switch to app '{}'", app_name));
            }
        }
    ...
    pub fn write_temp_file(content: &str, extension: &str) -> Result<String> {
        let path = crate::paths::temp_file("steer_bridge", extension);
        std::fs::write(&path, content)?;
        println!("[Bridge] Wrote temp file: {}", path.display());
...
pub fn calculate(expression: &str, ctx: &mut ExecutionContext) -> Result<ToolResult> {
    // Use bc for calculation (Linux/macOS)
