...
impl VisualDriver {
    ...
    fn capture_image_internal() -> Result<image::DynamicImage> {
        let output_path = crate::paths::temp_file("steer_vision", "jpg");
...
"@
                $win = [Win32]::GetForegroundWindow()
                $rectOk = $false
                $rect = New-Object Win32+RECT
                if ($win -ne [IntPtr]::Zero) {{
                    $rectOk = [Win32]::GetWindowRect($win, [ref]$rect)
                }}
                if ($rectOk) {{
                    ...
                    $screen = [System.Windows.Forms.Screen]::FromHandle($win)
            ...
            match &step.action {
                ...
                UiAction::Click(_target) => {
                    ...
                    let task = tokio::task::spawn_blocking(move || {
                        // Stub for named Click on Windows
                        warn!("Named Click not fully supported on Windows. Use ClickVisual.");
                        Ok::<String, anyhow::Error>("Scoped click not implemented".to_string())
                    });
                ...
                UiAction::Type(text) => {
                    ...
                    let _calc_like = !compact.is_empty()
                        && compact
                            .chars()
                    ...
                    let task = tokio::task::spawn_blocking(move || {
                        ...
                        #[cfg(target_os = "windows")]
                        {
                            let script = format!(
                                ...
                                text_clone.replace("'", "''") // Escape single quotes
                            );
                            let status = Command::new("powershell")
                                .args(["-NoProfile", "-Command", &script])
                                .status();
                            match status {
                                Ok(s) if s.success() => Ok::<String, anyhow::Error>("ok".to_string()),
                                _ => Err(anyhow::anyhow!("Win Type Failed")),
                            }
                        }
                    });
                ...
                UiAction::Scroll(direction) => {
                    ...
                    #[cfg(target_os = "windows")]
                    let task = tokio::task::spawn_blocking(move || {
                        let key = if dir == "up" { "{PGUP}" } else { "{PGDN}" };
                        let ps_script = format!("Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.SendKeys]::SendWait('{}')", key);
                        let _ = Command::new("powershell").args(["-NoProfile", "-Command", &ps_script]).status();
                ...
                UiAction::ActivateApp(app) => {
                    ...
                    let task = tokio::task::spawn_blocking(move || {
                        ...
                        #[cfg(target_os = "windows")]
                        {
                            let cmd = format!("(New-Object -ComObject WScript.Shell).AppActivate('{}')", app_name);
                            let _ = Command::new("powershell").args(["-NoProfile", "-Command", &cmd]).status();
                            Ok::<String, anyhow::Error>("ok".to_string())
                        }
                    });
                    match tokio::time::timeout(std::time::Duration::from_secs(5), task).await {
                ...
                UiAction::KeyboardShortcut(key, modifiers) => {
                    ...
                    debug!("      🔍 [DEBUG] Script Execution: {}", script);
                                        ...
                                        Ok(Ok(Some((x_raw, y_raw)))) => {
                                            ...
                                            let click_task = tokio::task::spawn_blocking(move || {
                                                let ps_script = format!(
                                                    ...
                                                    "@
                                                    Add-Type -MemberDefinition $code -Name Mouse -Namespace Win32
                                                    Add-Type -AssemblyName System.Windows.Forms
