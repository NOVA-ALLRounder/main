...
impl N8nApi {
    ...
    fn npx_command() -> &'static str {
        // On Windows, `npx` resolution can fail depending on PATHEXT/context.
        // `npx.cmd` is the most reliable entrypoint.
        "npx.cmd"
    }
    ...
    fn n8n_user_folder() -> std::path::PathBuf {
        if let Ok(folder) = std::env::var("N8N_USER_FOLDER") {
        ...
        let home_n8n = std::env::var("HOME")
            .or_else(|_| std::env::var("USERPROFILE"))
            .ok()
            .map(|h| PathBuf::from(h).join(".n8n"));
        if let Some(path) = &home_n8n {
            if path.join("database.sqlite").exists() {
                return path.clone();
            }
        }

        let app_dir_n8n = crate::paths::app_dir().join("n8n");
        if app_dir_n8n.join("database.sqlite").exists() {
            return app_dir_n8n;
        }

        home_n8n.unwrap_or(app_dir_n8n)
    }
    ...
    pub async fn ensure_server_running(&self) -> Result<()> {
        let user_folder = Self::n8n_user_folder();
        std::fs::create_dir_all(&user_folder).map_err(|e| {
            anyhow::anyhow!(
                "Failed to create N8N_USER_FOLDER {}: {}",
                user_folder.display(),
                e
            )
        })?;
        ...
        let _child = Command::new(Self::npx_command())
            .args(["-y", "n8n", "start", "--tunnel"])
            .env("N8N_USER_FOLDER", &user_folder)
    ...
    pub async fn create_workflow(
        ...
        if !self.api_key.is_empty() && self.api_key != "placeholder" {
            ...
            println!("?�️ No API Key configured. Using CLI mode.");
        }
        ...
        if let Ok(latest_after) = self.retrieve_latest_workflow_id() {
            if latest_before.as_ref() != Some(&latest_after) || latest_before.is_none() {
                println!(
                    "[n8n] Workflow imported via CLI; using latest workflow id fallback: {}",
                    latest_after
                );
        ...
        println!(
            "[n8n] Workflow imported via CLI but workflow ID lookup failed. Returning synthetic id."
        );
    ...
    async fn create_workflow_cli(
        ...
        let path = crate::paths::temp_file("n8n_import", "json");
        tokio::fs::write(&path, serde_json::to_string(&final_json)?).await?;

        println!("[n8n] Importing workflow via CLI from {}...", path.display());
    ...
    fn candidate_n8n_db_paths(&self) -> Vec<PathBuf> {
        ...
        if let Ok(folder) = std::env::var("N8N_USER_FOLDER") {
            let trimmed = folder.trim();
            if !trimmed.is_empty() {
                paths.push(PathBuf::from(trimmed).join("database.sqlite"));
            }
        }

        paths.push(Self::n8n_user_folder().join("database.sqlite"));
