...
impl Scheduler {
    ...
    pub fn start(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let due = match db::get_due_routines() {
                    Ok(r) => r,
                    Err(e) => {
                        eprintln!("?좑툘 Scheduler DB Error: {}", e);
                        continue;
                    }
                };
                ...
                for routine in due {
                    let permit = match semaphore.clone().acquire_owned().await {
                        Ok(p) => p,
                        Err(e) => {
                            eprintln!("?좑툘 Semaphore acquire failed: {}", e);
                            continue;
                        }
                    };
                    ...
                    tokio::spawn(async move {
                        ...
                        println!("   ?띰툘 running routine logic: '{}'...", prompt);
                        ...
                        loop {
                            match planner.run_goal(&prompt, None).await {
                                ...
                                Err(e) => {
                                    ...
                                    println!(
                                        "?좑툘 Routine '{}' attempt {} failed. Retrying in {}s...",
        ...
        });        let disable_background = std::env::var("STEER_DISABLE_BACKGROUND_ANALYSIS")
            .ok()
            .map(|v| matches!(v.trim().to_lowercase().as_str(), "1" | "true" | "yes" | "on"))
            .unwrap_or(false);

        if disable_background {
            println!("Background analysis disabled via STEER_DISABLE_BACKGROUND_ANALYSIS.");
        } else {
