...
pub fn spawn_app_watcher(log_tx: mpsc::Sender<String>) {
    std::thread::spawn(move || {
        ...
        loop {
            ...
            {
                if let Some((current_app, window_title)) = windows_frontmost_context() {
                    if !current_app.is_empty() && current_app != last_app {
                        last_app = current_app.clone();
                        let resource = ResourceContext {
                            resource_type: "app".to_string(),
...
#[cfg(target_os = "windows")]
fn windows_frontmost_context() -> Option<(String, String)> {
    let script = r#"
...
"#;
    ...
    let app = v.get("app").and_then(|x| x.as_str()).unwrap_or("").to_string();
    let title = v
        .get("title")
        .and_then(|x| x.as_str())
        .unwrap_or("")
