...
def _run_post_collection(config) -> None:
    ...
    try:
        ...
        if post.run_pattern_summary:
            ...
            subprocess.run(cmd, check=False)
        llm_input_path = str(Path(output_dir) / "llm_input.json")
        workflow_output_path = str(Path(output_dir) / "n8n_workflow.json")
        if post.run_llm_input:
            ...
            subprocess.run(cmd, check=False)
        if post.run_n8n_workflow:
            profile_path = (
                str(project_root / post.n8n_profile_path)
                if post.n8n_profile_path
                else str(project_root / "configs/personalization_demo.json")
            )
            score_cfg_path = (
                str(project_root / post.n8n_score_config_path)
                if post.n8n_score_config_path
                else str(project_root / "configs/score_config.json")
            )
            if post.run_n8n_quality_loop:
                cmd = [
                    python_exe,
                    str(scripts_dir / "generate_workflow_with_retry.py"),
                    "--config",
                    str(config.config_path),
                    "--input",
                    llm_input_path,
                    ...
                    score_cfg_path,
                    "--min-score",
                    str(int(post.n8n_min_score)),
                    "--max-attempts",
                    str(int(post.n8n_max_attempts)),
                ]
            else:
                cmd = [
                    python_exe,
                    str(scripts_dir / "generate_n8n_workflow.py"),
                    "--config",
                    str(config.config_path),
                    "--input",
                    llm_input_path,
            ...
            if post.send_n8n_workflow:
                webhook = (post.n8n_webhook_url or "").strip()
                if not webhook:
                    webhook_env = (post.n8n_webhook_env or "N8N_WEBHOOK_URL").strip()
                    webhook = os.getenv(webhook_env, "").strip()
                if webhook:
                    cmd = [
                        python_exe,
                        str(scripts_dir / "send_n8n_workflow.py"),
                        "--file",
                        workflow_output_path,
                        "--webhook",
                        webhook,
                    ...
                    logger.warning(
                        "post_collection send_n8n_workflow=true but webhook is empty"
                    )
