...
1.  **존재하지 않는 앱/데이터에 대해 성공 판정**:
    *   **Excel**: 고객님 맥북에는 엑셀이 없는데 "Excel 열기 성공"이라고 보고함.
...
**고객님의 피드백을 수용하여 `core/src/reality_check.rs`를 구현했습니다.**
...
**문제 상황:** 앱은 열었으나 "새 메모"를 만들지 않고 타이핑하여 실제 내용이 저장되지 않음.
**수정 사항:**
1.  **Keyboard Shortcut 구현**: `cmd + n` 단축키 지원을 `core` 엔진에 추가.
2.  **System Prompt 강화**: "앱을 열면 무조건 `Shortcut(Cmd + N)`을 먼저 수행하라"는 규칙 주입.

**최종 결과 (Scenario 1 Re-run)**:
1.  `Open App: Notes`
2.  **`Shortcut: n + ["command"]` (새 메모 생성 성공)**
3.  `Type: "오늘의 브리핑..."`
4.  **Result: 성공 (실제 메모 생성 확인)**

## 2️⃣ 시나리오 2: 시장 조사 (Market Research) - 재검증 결과
**명령:** Safari 검색 -> Excel 기록
**결과 (Honest Mode)**:
