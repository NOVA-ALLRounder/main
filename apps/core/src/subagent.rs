...
impl SubagentManager {
    ...
    pub fn get(id: &str) -> Option<Subagent> {
        SUBAGENT_REGISTRY.lock()
            .ok()
            .and_then(|r| r.get(id).cloned())
    }
    
    /// Update subagent status
    pub fn update_status(id: &str, status: SubagentStatus, result: Option<String>, error: Option<String>) {
        if let Ok(mut registry) = SUBAGENT_REGISTRY.lock() {
            if let Some(agent) = registry.get_mut(id) {
                ...
                if matches!(status, SubagentStatus::Completed | SubagentStatus::Failed | SubagentStatus::Cancelled) {
    ...
    pub fn complete(id: &str, result: &str) {
        Self::update_status(id, SubagentStatus::Completed, Some(result.to_string()), None);
    ...
    pub fn cancel(id: &str) {
        Self::update_status(id, SubagentStatus::Cancelled, None, Some("Cancelled by user".to_string()));
    ...
    pub fn list_active() -> Vec<Subagent> {
        SUBAGENT_REGISTRY.lock()
            .map(|r| {
                r.values()
                    .filter(|a| matches!(a.status, SubagentStatus::Pending | SubagentStatus::Running))
                    .cloned()
                    .collect()
            })
            .unwrap_or_default()
    ...
    pub async fn wait_for(id: &str, timeout_ms: u64) -> Option<Subagent> {
        ...
        loop {
            if let Some(agent) = Self::get(id) {
                if matches!(agent.status, SubagentStatus::Completed | SubagentStatus::Failed | SubagentStatus::Cancelled) {
                    return Some(agent);
                }
            }
    ...
    pub fn cleanup_old(max_age_secs: u64) {
        if let Ok(mut registry) = SUBAGENT_REGISTRY.lock() {
            ...
            registry.retain(|_, a| {
                if matches!(a.status, SubagentStatus::Completed | SubagentStatus::Failed | SubagentStatus::Cancelled) {
