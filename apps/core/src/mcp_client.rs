...
impl McpClient {
    ...
    pub fn connect(&mut self) -> Result<()> {
        ...
        let child = cmd.spawn()
            .context(format!("Failed to start MCP server: {}", self.server.command))?;
        ...
        println!("✅ [MCP] Connected: {} tools, {} resources", 
...
impl McpRegistry {
    ...
    fn create_default_config(&self) -> Result<()> {
        ...
        println!("📝 [MCP] Created default config: {}", self.config_path.display());
...
pub fn init_mcp() -> Result<()> {
    let mut guard = MCP_REGISTRY.lock().map_err(|e| anyhow::anyhow!("Lock error: {}", e))?;
...
pub fn get_mcp_registry() -> Result<std::sync::MutexGuard<'static, Option<McpRegistry>>> {
    MCP_REGISTRY.lock().map_err(|e| anyhow::anyhow!("Lock error: {}", e))
}

/// Call an MCP tool
pub fn call_mcp_tool(server: &str, tool: &str, args: Value) -> Result<Value> {
    let mut guard = get_mcp_registry()?;
    let registry = guard.as_mut().ok_or_else(|| anyhow::anyhow!("MCP not initialized"))?;
