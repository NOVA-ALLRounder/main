...
fn safe_files_txt_command() -> &'static str {
    #[cfg(target_os = "windows")]
    {
        "Get-ChildItem -Force | Where-Object { $_.Name -ne 'files.txt' -and -not $_.Name.StartsWith('.') } | Select-Object -ExpandProperty Name | Sort-Object | Set-Content -Encoding UTF8 files.txt"
    }
...
fn normalize_ls_command(instr: &str) -> String {
    if cfg!(target_os = "windows") {
        ...
        if trimmed.starts_with("ls") {
            let has_one = trimmed.contains("-1");
            let replacement = if has_one { "Get-ChildItem -Name" } else { "Get-ChildItem" };
            let re = regex::Regex::new(r"^ls(\s+-[a-zA-Z0-9]+)*").unwrap();
            return re.replace(instr, replacement).to_string();
        }
