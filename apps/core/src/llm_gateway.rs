...
<<<<<<< HEAD:apps/core/src/llm_gateway.rs
    /// Dynamically build context for workflow generation based on available integrations & tools
    fn get_workflow_context(&self) -> String {
        let mut context = String::from("## AVAILABLE NODES\n");
        ...
        if cfg!(target_os = "macos") {
            // Check for 'cliclick' (better mouse control)
            let has_cliclick = command_exists("cliclick");
            ...
            let has_ahk = command_exists("AutoHotkey.exe") || command_exists("AutoHotkey");
            if has_ahk {
                context.push_str("- ✅ UI CONTROL: AutoHotkey is installed.\n");
                context.push_str("  - Example: `Click 200, 300` / `Send \"text\"` (use a .ahk script)\n");
            } else {
                context.push_str("- ⚠️ UI CONTROL: AutoHotkey is NOT installed.\n");
                context.push_str("  - Recommend installing AutoHotkey for reliable mouse/keyboard control.\n");
            }
            context.push_str("- ✅ APP LAUNCH: Use PowerShell `Start-Process` for opening apps/URLs.\n\n");
        } else {
            context.push_str("- ⚠️ UI automation may be limited on this platform.\n\n");
        }
...
>>>>>>> origin/steer/develop:core/src/llm_gateway.rs
        if logs.is_empty() {
