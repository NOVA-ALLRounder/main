...
pub fn spawn(
    ...
    tokio::spawn(async move {
        ...
        let mut dcp_buffer: Vec<EventEnvelope> = Vec::new();
        ...
        while let Some(log_json) = log_rx.recv().await {
            ...
            if let Ok(event) = serde_json::from_str::<EventEnvelope>(&log_json) {
                ...
                if let Some(masked_event) = guard.apply(event) {
                                    ...
                                    if let Some(mem) = &memory {
                                        ...
                                        match tokio::fs::read_to_string(path).await {
                                            Ok(content) => {
                                                 ...
                                                 println!("📄 [Analyzer] Ingesting file update: {}", path);
                                                 if let Err(e) = mem.add(&content, meta).await {
                                                     eprintln!("⚠️ [Analyzer] Mem Ingest Failed: {}", e);
                                                 }
                                            },
                                            Err(e) => eprintln!("⚠️ [Analyzer] Failed to read file {}: {}", path, e),
                                        }
                                    }
                                }
                            }
                    ...
                    // 3. Persist or Forward
                    if use_dcp {
                        dcp_buffer.push(masked_event.clone());
                    } else if let Err(e) = db::insert_event_v2(&masked_event) {
                    ...
                    if session_buffer.len() >= batch_size || last_process_at.elapsed() >= max_buffer_age || is_idle {
                        if use_dcp && !dcp_buffer.is_empty() {
                            if let Err(e) = collector_bridge::send_events(&dcp_buffer).await {
                                eprintln!("⚠️ [Analyzer] DCP Forward Error: {}", e);
                            } else {
                                dcp_buffer.clear();
                            }
                        }
                        process_buffer(&mut session_buffer, &sessionizer, &detector, &matcher, &memory, &llm_client, use_dcp).await;
                 ...
                 eprintln!("⚠️ [Analyzer] Failed to parse log as EventEnvelope: {}", log_json);
            }
        }
    });
}
