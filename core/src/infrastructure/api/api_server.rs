...
async fn handle_chat(
    ...
    if message == "n8n restart" || message.contains("n8n ?ъ떆??) {
    ...
    if message.trim() == "/capture" || message.contains("?붾㈃ 遺꾩꽍?댁쨾") || message.contains("analyze screen") {
         if let Some(llm) = &state.llm_client {
             match crate::visual_driver::VisualDriver::capture_screen() {
                 Ok((b64, _scale)) => {
                     ...
                     match llm.analyze_screen(prompt, &b64).await {
                         Ok(desc) => return Json(ChatResponse { response: format!("?몓截??붾㈃ 遺꾩꽍 寃곌낵:\n{}", desc), command: None }),
                         Err(e) => return Json(ChatResponse { response: format!("??Vision API ?ㅻ쪟: {}", e), command: None }),
                     }
                 },
                 Err(e) => return Json(ChatResponse { response: format!("???붾㈃ 罹≪쿂 ?ㅽ뙣: {}", e), command: None }),
             }
         } else {
             return Json(ChatResponse { response: "??LLM ?대씪?댁뼵?멸? 珥덇린?붾릺吏 ?딆븯?듬땲??".to_string(), command: None });
         }
    }
            ...
            Ok(intent) => {
                ...
                let response = match command.as_str() {
                    ...
                    "build_workflow" => {
                        ...
                        let response_msg = format!("?룛截??먮룞???뚰겕?뚮줈???앹꽦???쒖옉?⑸땲?? '{}'\n(?좎떆留?湲곕떎?ㅼ＜?몄슂...)", prompt);
                            ...
                            match n8n_api::N8nApi::from_env() {
                                Ok(n8n) => {
                                    ...
                                    if let Err(e) = n8n.ensure_server_running().await {
                                        println!("?좑툘 n8n Server Check Failed: {}", e);
...
async fn approve_recommendation(
    ...
    let mut current_json = if let Some(json) = &rec.workflow_json {
        ...
        if let Some(llm) = &state.llm_client {
             ...
             println!("?쭬 Generating workflow with context: {} credentials", credentials.len());
    ...
    while attempts < max_attempts {
        ...
        if attempts > 1 {
            if let Some(llm) = &state.llm_client {
                println!("?뵩 Attempting to fix workflow JSON (Try {}/{})", attempts, max_attempts);
