...
/// Get a connection from the pool (public for cross-module access)
...
pub fn init() -> anyhow::Result<()> {
    ...
    // Create connection manager with WAL mode initialization
    ...
    println!("?踰 Database initialized at: {:?}", db_path);

    // Get a connection from pool to run schema initialization
    ...
    // Store pool globally
    {
        let mut lock = DB_POOL.write().unwrap();
        *lock = Some(pool);
    }

    println!("?踰 Database 'steer.db' initialized.");
    ...
    // Init JARVIS Schema
    if let Err(e) = init_jarvis() {
        eprintln!("Failed to init JARVIS tables: {}", e);
    }

    Ok(())
}

/// Initialize JARVIS-specific tables
fn init_jarvis() -> Result<()> {
    let conn = get_connection()?;

    // Activity logs (Context Engine)
    conn.execute(
        "CREATE TABLE IF NOT EXISTS jarvis_activity_logs (
            ...
            window_title TEXT,
            metadata TEXT,
            privacy_mode TEXT NOT NULL DEFAULT 'basic'
        )",
        [],
    )?;
    ...
    conn.execute(
        "CREATE TABLE IF NOT EXISTS jarvis_patterns (
            id TEXT PRIMARY KEY,
            pattern_type TEXT NOT NULL,
    ...
    // Automation suggestions (Workflow Builder)
    conn.execute(
        "CREATE TABLE IF NOT EXISTS jarvis_automation_suggestions (
            id TEXT PRIMARY KEY,
            ...
            FOREIGN KEY (pattern_id) REFERENCES jarvis_patterns(id)
        )",
        [],
    )?;
    ...
    conn.execute(
        "CREATE TABLE IF NOT EXISTS jarvis_workflows (
            ...
            automation_suggestion_id TEXT,
            created_by TEXT NOT NULL DEFAULT 'user',
            status TEXT NOT NULL DEFAULT 'draft',
            created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
            updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (automation_suggestion_id) REFERENCES jarvis_automation_suggestions(id)
        )",
        [],
    )?;

    // Memories (Memory Tool)
    conn.execute(
    ...
    // Vision cache (Vision Engine)
    conn.execute(
        "CREATE TABLE IF NOT EXISTS jarvis_vision_cache (
    ...
    // Create indices
    conn.execute("CREATE INDEX IF NOT EXISTS idx_jarvis_activity_timestamp ON jarvis_activity_logs(timestamp)", [])?;
    conn.execute("CREATE INDEX IF NOT EXISTS idx_jarvis_activity_event_type ON jarvis_activity_logs(event_type)", [])?;
    conn.execute("CREATE INDEX IF NOT EXISTS idx_jarvis_activity_app ON jarvis_activity_logs(app_name)", [])?;
    conn.execute("CREATE INDEX IF NOT EXISTS idx_jarvis_pattern_type ON jarvis_patterns(pattern_type)", [])?;
    conn.execute("CREATE INDEX IF NOT EXISTS idx_jarvis_pattern_status ON jarvis_patterns(status)", [])?;
    conn.execute("CREATE INDEX IF NOT EXISTS idx_jarvis_suggestion_status ON jarvis_automation_suggestions(status)", [])?;
    conn.execute("CREATE INDEX IF NOT EXISTS idx_jarvis_session_state ON jarvis_sessions(state)", [])?;

    // Create triggers for auto-cleanup
    conn.execute(
        "CREATE TRIGGER IF NOT EXISTS jarvis_cleanup_old_activities
        AFTER INSERT ON jarvis_activity_logs
        BEGIN
            DELETE FROM jarvis_activity_logs
    ...
    conn.execute(
        "CREATE TRIGGER IF NOT EXISTS jarvis_cleanup_expired_memories
        AFTER INSERT ON jarvis_memories
        BEGIN
    ...
    conn.execute(
        "CREATE TRIGGER IF NOT EXISTS jarvis_cleanup_vision_cache
        AFTER INSERT ON jarvis_vision_cache
        BEGIN
            DELETE FROM jarvis_vision_cache
            WHERE expires_at < CURRENT_TIMESTAMP;
        END",
        [],
    )?;

    // Vector Memory Store (Digital Self)
    conn.execute(
        "CREATE TABLE IF NOT EXISTS jarvis_vector_memories (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            content TEXT NOT NULL,
    ...
    // Conversation summaries (Digital Self)
    conn.execute(
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_jarvis_vmem_category ON jarvis_vector_memories(category)", [])?;
    conn.execute("CREATE INDEX IF NOT EXISTS idx_jarvis_vmem_importance ON jarvis_vector_memories(importance)", [])?;
    conn.execute("CREATE INDEX IF NOT EXISTS idx_jarvis_convsumm_session ON jarvis_conversation_summaries(session_key)", [])?;

    println!("??JARVIS tables initialized successfully");
...
pub fn save_session(
    ...
    conn.execute(
        "INSERT OR REPLACE INTO jarvis_sessions
         (session_key, context, state, created_at, last_activity)
         VALUES (?1, ?2, ?3, ?4, ?5)",
        params![session_key, context_json, state, created_at, last_activity],
    )?;
    Ok(())
}

/// Load all active/idle sessions from database
pub fn load_sessions() -> Result<Vec<(String, String, String, i64, i64)>> {
    let conn = get_connection()?;
    let mut stmt = conn.prepare(
        "SELECT session_key, context, state, created_at, last_activity
    ...
    let sessions = stmt.query_map([], |row| {
        Ok((
            row.get(0)?, // session_key
            row.get(1)?, // context
            row.get(2)?, // state
            row.get(3)?, // created_at
            row.get(4)?, // last_activity
        ))
    })?;
...
pub fn delete_session(session_key: &str) -> Result<()> {
    let conn = get_connection()?;
    conn.execute(
        "DELETE FROM jarvis_sessions WHERE session_key = ?1",
        params![session_key],
    )?;
    Ok(())
}
...
pub fn seed_advanced_examples() -> Result<()> {
    ...
    conn.execute(
        ...
        params![
            ...
            "筌띲끉???袁⑸쵟 9??뽯퓠 ??깆젟???醫롫뎁???遺용튋??곴퐣 ?遺얠쟿域밸챶???곗쨮 癰귣?源??덈뼄.",
    ...
    let urgent_mail_json = r#"{
            ...
            "nodes": [
                ...
                { "type": "n8n-nodes-base.if", "typeVersion": 1, "position": [300, 300], "parameters": { "conditions": { "string": [{ "value1": "={{ $json.snippet }}", "operation": "contains", "value2": "urgent" }, { "value1": "={{ $json.subject }}", "operation": "contains", "value2": "疫뀀떯?? }] }, "combineOperation": "any" }, "name": "Is Urgent?" },
    ...
    conn.execute(
        ...
        params![
            ...
            "pending",
            "???疫뀀떯??筌롫뗄?????뵝",
            "??뺛걠??援???곸뒠??'疫뀀떯??????釉??筌롫뗄?????삠늺 筌앸맩?????뵝??癰귣?源??덈뼄.",
            "New Email in Inbox",
            "[\"Check Keywords\", \"Telegram Alert\"]",
            "Watch Gmail for new emails. If subject contains 'urgent' or '疫뀀떯??, send a Telegram notification.",
            "seed_urgent_001",
            0.95,
            urgent_mail_json
        ],
