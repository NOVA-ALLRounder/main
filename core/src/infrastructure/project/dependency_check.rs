...
impl SystemHealth {
    pub fn check_all() -> Self {
        ...
        #[cfg(target_os = "macos")]
        {
            deps.push(Dependency::new("Homebrew", "which brew", "/bin/bash -c \"$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)\"", true));
            deps.push(Dependency::new("cliclick", "which cliclick", "brew install cliclick", false));
        }

        #[cfg(target_os = "windows")]
        {
            // Windows-specific tools (none required for now)
            // PowerShell and cmd are built-in
        }

        // Common dependencies (all platforms)
        #[cfg(target_os = "windows")]
        deps.push(Dependency::new("n8n", "where n8n", "npm install -g n8n", false)); // Windows uses 'where'

        #[cfg(not(target_os = "windows"))]
        deps.push(Dependency::new("n8n", "which n8n", "npm install -g n8n", false)); // Unix uses 'which'
    ...
    pub fn print_report(&self) {
        ...
        println!("?醫묓닔  MISSING DEPENDENCIES DETECTED:");
