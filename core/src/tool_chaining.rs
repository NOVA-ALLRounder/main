...
impl ToolResult {
    ...
    pub fn with_data(mut self, key: &str, value: &str) -> Self {
        self.extracted_data.insert(key.to_string(), value.to_string());
        self
    }
...
impl CrossAppBridge {
    ...
    pub fn copy_to_clipboard(text: &str) -> Result<()> {
        ...
        println!("📋 [Bridge] Copied to clipboard: {}...", &text[..text.len().min(50)]);
        Ok(())
    }
...
pub fn read_from_app(app_name: &str, ctx: &mut ExecutionContext) -> Result<ToolResult> {
    ...
    Ok(ToolResult::success("read_from_app", serde_json::json!({
        "app": app_name,
        "content": content
...
pub fn write_to_app(app_name: &str, content: &str, ctx: &mut ExecutionContext) -> Result<ToolResult> {
    ...
    Ok(ToolResult::success("write_to_app", serde_json::json!({
        "app": app_name,
...
pub fn calculate(expression: &str, ctx: &mut ExecutionContext) -> Result<ToolResult> {
    ...
    let output = std::process::Command::new("bc")
        .arg("-l")
    ...
    Ok(ToolResult::success("calculate", serde_json::json!({
