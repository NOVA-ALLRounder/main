    ...
    pub fn start(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let due = match db::get_due_routines() {
                    Ok(r) => r,
                    Err(e) => {
                        eprintln!("?醫묓닔 Scheduler DB Error: {}", e);
                        continue;
                    }
                };
                ...
                for routine in due {
                    let permit = match semaphore.clone().acquire_owned().await {
                        Ok(p) => p,
                        Err(e) => {
                            eprintln!("?醫묓닔 Semaphore acquire failed: {}", e);
                            continue;
                        }
                    };
                    ...
                    tokio::spawn(async move {
                        ...
                        println!("   ??고닔 running routine logic: '{}'...", prompt);
                        ...
                        loop {
                            match executor.execute_goal(&prompt).await {
                                ...
                                Err(e) => {
                                    ...
                                    println!("?醫묓닔 Routine '{}' attempt {} failed. Retrying in {}s...", prompt, attempt, retry_delay_secs);
        ...
        tokio::spawn(async move {
            loop {
                ...
                println!("?彛?[Background] Analyzing recent behavior patterns...");
