...
impl VisualDriver {
    ...
    fn capture_image_internal() -> Result<image::DynamicImage> {
        // Use platform-agnostic screen capture
        let platform = crate::platform::get_platform();
        let jpeg_bytes = platform.screen_capture().capture_screen()
            .context("Failed to capture screen")?;

        image::load_from_memory(&jpeg_bytes).context("Failed to load image")
    }
    ...
    async fn verify_condition(llm: &crate::llm_gateway::LLMClient, prompt: &str) -> Result<bool> {
        println!("      ?紐볠닼?Vision Check: '{}'", prompt);
            ...
            Ok((b64, _scale)) => { // Ignore scale for verification
                ...
                match llm.analyze_screen(&full_prompt, &b64).await {
                    Ok(resp) => {
                        ...
                        println!("      ?夷?Result: {}", if success { "PASS" } else { "FAIL" });
                        Ok(success)
                    },
                    Err(e) => {
                        println!("      ?醫묓닔 Vision API Error: {}", e);
                        Ok(false) // Conservative failure
                    }
                }
            },
            Err(e) => {
                println!("      ?醫묓닔 Capture Failed: {}", e);
                Ok(false)
            }
        }
    }
    ...
    pub async fn execute(&self, llm: Option<&crate::llm_gateway::LLMClient>) -> Result<()> {
        println!("?維?[Smart Visual Driver] Starting Verified Automation...");
        ...
        for (i, step) in self.steps.iter().enumerate() {
                ...
                if let Some(brain) = llm {
                    if !Self::verify_condition(brain, pre_prompt).await? {
                         if step.critical {
                             ...
                             println!("      ?醫묓닔 Pre-check failed, but proceeding (non-critical).");
                         }
                    }
                }
            }
            ...
            match &step.action {
                ...
                UiAction::ClickVisual(desc) => {
                    println!("      ?紐볠닼?Vision Click: Finding '{}'...", desc);
                    if let Some(brain) = llm {
                         let max_retries = 2;
                                        ...
                                        Ok(Some((x_raw, y_raw))) => {
                                            ...
                                            #[cfg(target_os = "macos")]
                                            {
                                                ...
                                                if let Some((_sx, _sy)) = accessibility::get_element_center_at(x, y) {
                                                    ...
                                                    println!("      ?醫묓닔  Warning: No UI Element found at coordinates via Accessibility API.");
                        ...
                        println!("      ?醫묓닔 No LLM client provided for Visual Click.");
                    }
                }
            }
        ...
        println!("?維?[Smart Visual Driver] Automation Complete.");
        Ok(())
    }
}
