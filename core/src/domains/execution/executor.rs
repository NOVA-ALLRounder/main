...
impl AgentExecutor {
    ...
    pub async fn execute_goal(&self, goal: &str) -> Result<String> {
        println!("?彛?[OODA] Goal received: '{}'", goal);
        ...
        println!("?彛?[OODA] Plan generated with {} steps.", plan.len());
        ...
        'outer: while step_index < plan.len() {
            ...
            println!("?彛?[OODA] Executing Step {}: {}", step_index + 1, step.description);
            ...
            while attempts <= max_retries {
                ...
                step_driver.add_step(smart_step.clone());
                    ...
                    Err(e) => {
                        ...
                        println!("?醫묓닔 Step {} Failed [{}] (Attempt {}/{}): {}", step_index + 1, failure_type, attempts, max_retries + 1, e);
                        last_error = Some(e);
                        
                        if failure_type == "permission_denied" {
                            println!("?酉귦닔 Critical Permission Error. Aborting Self-Healing to prevent spamming OS.");
            ...
            if strategy.stop {
                println!("?酉귦닔 Replan stopped: {}", strategy.reason);
