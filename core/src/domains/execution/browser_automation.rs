...
pub fn fill_flight_fields(from: &str, to: &str, date_start: &str, date_end: Option<&str>) -> Result<bool> {
    ...
    let js = format!(
        r#"(() => {{
            ...
            if (isGoogleFlights) {{
                const fromInput =
                    document.querySelector('input[aria-label*="Where from"], input[aria-label*="From"], input[placeholder*="Where from"], input[aria-label*="?곗뮆而?]') ||
                    findInputByTokens(['where from', 'from', '?곗뮆而?, '?곗뮆而삼쭪?']);
                if (fillInput(fromInput, values.from)) filled++;
                const toInput =
                    document.querySelector('input[aria-label*="Where to"], input[aria-label*="To"], input[placeholder*="Where to"], input[aria-label*="?袁⑷컩"]') ||
                    findInputByTokens(['where to', 'to', '?袁⑷컩', '?袁⑷컩筌왖']);
                if (fillInput(toInput, values.to)) filled++;
                const departInput =
                    document.querySelector('input[aria-label*="Departure"], input[aria-label*="Depart"], input[placeholder*="Departure"], input[aria-label*="?곗뮆而?], input[aria-label*="揶쎛?遺얘텊"]') ||
                    findInputByTokens(['departure', 'depart', '?곗뮆而?, '揶쎛?遺얘텊', '?醫롮?', '?곗뮆而??]);
                if (fillInput(departInput, values.date_start)) filled++;
                const returnInput =
                    document.querySelector('input[aria-label*="Return"], input[aria-label*="??삳뮉??], input[aria-label*="域뮤??], input[placeholder*="Return"]') ||
                    findInputByTokens(['return', '??삳뮉??, '域뮤??, '癰귣벀?']);
                if (fillInput(returnInput, values.date_end)) filled++;
            }}
            if (filled === 0) {{
                const matchers = [
                    ['from', ['from', '?곗뮆而?, '?곗뮆而삼쭪?', 'where from']],
                    ['to', ['to', '?袁⑷컩', '?袁⑷컩筌왖', 'where to']],
                    ['date_start', ['depart', '?곗뮆而??, '?醫롮?', 'date']]
                ];
                for (const input of inputs) {{
                    const label = (input.getAttribute('aria-label') || input.getAttribute('placeholder') || input.name || input.id || '').toLowerCase();
    ...
    let js = format!(
        r#"(() => {{
            ...
            const input = document.querySelector(
                'input[name=\"query\"], input[name=\"searchKeyword\"], input#query, input#autocomplete, input[type=\"search\"], input[placeholder*=\"野꺜??"], input[placeholder*=\"Search\"], input[id*=\"search\"]'
            );
            if (!input) return '0';
            input.focus();
            const setter =
...
pub fn click_search_button() -> Result<bool> {
    ...
    let js = r#"(() => {
        ...
        const tokens = ['野꺜??, 'search', '筌≪뼐由?, 'submit'];
...
pub fn autofill_form(name: Option<&str>, email: Option<&str>, phone: Option<&str>, address: Option<&str>) -> Result<bool> {
    ...
    let js = format!(
        r#"(() => {{
            ...
            let filled = 0;
            const rules = [
                ['name', ['name', '??已?, '?源낆구', 'customer']],
                ['email', ['email', '??李??, 'e-mail']],
                ['phone', ['phone', 'telephone', 'tel', '?袁れ넅', '?怨뺤뵭', '?????, 'mobile']],
                ['address', ['address', '雅뚯눘??]]
            ];
            const setNativeValue = (el, value) => {{
                const setter =
...
pub fn extract_flight_summary() -> Result<String> {
    ...
    let js = r#"(() => {
        ...
        const times = [];
        const stops = [];
        const priceRe = /(\d{1,3}(?:,\d{3})+)\s*??\$\d+/;
        const timeRe = /(\d{1,2}:\d{2})|(\d{1,2})\s*??볦퍢\s*(\d{1,2})?\s*??/;
        const stopRe = /(筌욊낱鍮?野껋럩?|stop|stops?)/i;
...
pub fn extract_shopping_summary() -> Result<String> {
    ...
    let js = r#"(() => {
        const text = document.body?.innerText || '';
        const lines = text.split('\n').map(l => l.trim()).filter(Boolean);
        const priceNodes = Array.from(document.querySelectorAll('[class*="price"], [class*="amount"], [aria-label*="??], [aria-label*="$"]'))
            .map(el => (el.innerText || el.getAttribute('aria-label') || '').trim())
            .filter(Boolean);
        const pool = [...priceNodes, ...lines];
        const prices = [];
        const sellers = [];
        const priceRe = /(\d{1,3}(?:,\d{3})+)\s*??;
        for (const line of pool) {
            if (prices.length < 5) {
                const m = line.match(priceRe);
                if (m) {
            ...
            if (sellers.length < 5 && line.toLowerCase().includes('?癒?꼻')) {
    ...
    let js = format!(
        r#"(() => {{
            ...
            if (directOnly && ['true','1','yes','y'].includes(normalize(directOnly))) {{
                clickByTokens(['nonstop', 'direct', '筌욊낱鍮?]);
            }}
            if (timeWindow) {{
                const key = normalize(timeWindow);
                const tokensByKey = {{
                    morning: ['morning', '??쇱읈', 'am'],
                    afternoon: ['afternoon', '??쎌뜎', 'pm'],
                    evening: ['evening', 'night', '????, '獄?],
                }};
            ...
            if (budget) {{
                ...
                const priceInput = inputs.find(input => {{
                    ...
                    return label.includes('price') || label.includes('揶쎛野?) || label.includes('max') || label.includes('筌ㅼ뮆?');
                }});
                if (priceInput) {{
                    priceInput.focus();
                    const setter =
    ...
    let js = format!(
        r#"(() => {{
            ...
            const minInput = inputs.find(input => {{
                const label = normalize(input.getAttribute('aria-label') || input.getAttribute('placeholder') || input.name || input.id || '');
                return label.includes('min') || label.includes('筌ㅼ뮇??);
            }});
            const maxInput = inputs.find(input => {{
                const label = normalize(input.getAttribute('aria-label') || input.getAttribute('placeholder') || input.name || input.id || '');
                return label.includes('max') || label.includes('筌ㅼ뮆?');
            }});
