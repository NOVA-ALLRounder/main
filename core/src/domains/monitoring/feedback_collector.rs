...
pub enum ErrorCategory {
    ApiError,          // n8n API ??살첒
    AuthError,         // ?紐꾩쵄 ?얜챷??    SchemaError,       // ??롢걵????곌쾿???쨮????쎄텕筌?    TriggerError,      // ?紐꺿봺椰???쇱젟 ??살첒
    ActionError,       // ??る???쎈뻬 ??살첒
    Unknown,
}
...
pub struct FeedbackCollector {
    suppressed_patterns: Vec<String>, // ???젫?????쉘 ID 筌뤴뫖以?}
...
impl FeedbackCollector {
    ...
    pub fn record_success(&mut self, recommendation_id: i64, workflow_id: &str, execution_time_ms: u64) {
        ...
        println!("?諭?Feedback recorded: Success for workflow {}", workflow_id);
    }
    ...
    pub fn record_failure(&mut self, recommendation_id: i64, workflow_id: &str, error: &str) {
        ...
        println!("?諭?Feedback recorded: Failure for workflow {} - {}", workflow_id, error);
    }
    ...
    fn check_suppression(&mut self, recommendation_id: i64) {
        ...
        if fail_count >= 3 {
            ...
            println!("?醫묓닔  Recommendation {} suppressed due to repeated failures", recommendation_id);
        }
    }
