        ...
        loop {
                ...
                .await 
            {
                ...
                Err(e) => {
                    ...
                    eprintln!("?醫묓닔 LLM Network Error (Attempt {}/{}): {}. Retrying in {:?}...", attempt, max_retries, e, backoff);
                }
            }
...
"#;
    ...
    /// Chat completion with JSON mode and custom model support
    pub async fn chat_completion_json(&self, messages: Vec<Value>, model: Option<&str>) -> Result<String> {
...
"##, user_prompt, error_msg);
    ...
    fn get_workflow_context(&self) -> String {
        ...
        if has_cliclick {
            ...
            context.push_str("- ?醫묓닔 MOUSE CONTROL: 'cliclick' is NOT installed.\n");
...
- gmail_summary: Summarize recent emails. Params: count (number, default 10), hours (number, default 24)
...
- notion_append: Append to existing notion page. Params: page_id (optional), content
...
- open_browser: Open web browser. Params: url (URL or site name like "youtube", "google", "naver")
- open_app: Launch application. Params: app_name (e.g., "Chrome", "VS Code", "筌롫뗀???, "Excel", "Notion")
- type_text: Type text using keyboard. Params: text
- click_at: Click at screen position. Params: x, y (coordinates)
- press_key: Press keyboard key. Params: key (e.g., "enter", "escape", "F5")
- complex_task: Multi-step automation task. Params: description (full user request), steps (array of step descriptions)
- help: Show help. No params.
- unknown: Cannot parse. Params: original_text

Korean command examples:
- "?紐낃숲???녹뮇夷? / "?됰슢??怨? ??곷선餓? ??open_browser
- "?醫뤿뮔??????곸㉭" / "?醫뤿뮔????곷선餓? ??open_browser with url="youtube"
- "??쇱뵠甕???곷선餓? ??open_browser with url="naver"
- "??竊??녹뮇夷? ??open_app with app_name="Chrome"
- "?臾? ?녹뮇夷? ??open_app with app_name="Excel"
- "?紐꾨??녹뮇夷? ??open_app with app_name="Notion"

Complex task examples (use complex_task command):
- "筌왖??獄?筌롫뗄???類ㅼ뵥??랁??遺용튋??곴퐣 ?紐꾨???類ｂ봺??곸㉭" ??complex_task with steps: ["Check emails from last night", "Summarize email content", "Create Notion page with summary"]
- "?紐꾨??類ㅼ뵥??랁?筌롫뗄?ゆ에?癰귣떯???곸㉭" ??complex_task with steps: ["Open Notion", "Read content (manual)", "Send email report"]
...
Output JSON schema:
{
  ...
  "trigger": "What triggers this workflow (e.g., 'Gmail ????李???袁⑷컩', 'Downloads ????????뵬 ??밴쉐')",
