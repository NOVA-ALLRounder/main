...
pub fn spawn(
    ...
    tokio::spawn(async move {
        ...
        let salt = std::env::var("PRIVACY_SALT").unwrap_or_else(|_| {
            eprintln!("?醫묓닔 [Privacy] PRIVACY_SALT not set, using default. Set PRIVACY_SALT env var for production.");
        ...
        let memory = match MemoryStore::new(&mem_path, llm_client.clone()).await {
            Ok(m) => {
                println!("?彛?[Memory] Visual Cortex Online at: {}", mem_path);
                Some(m)
            },
            Err(e) => {
                eprintln!("?醫묓닔 [Memory] Failed to init Vector DB: {}", e);
                None
            }
        };
            ...
            if let Ok(event) = serde_json::from_str::<EventEnvelope>(&log_json) {
                ...
                if let Some(masked_event) = guard.apply(event) {
                                    ...
                                    if let Some(mem) = &memory {
                                        ...
                                        match tokio::fs::read_to_string(path).await {
                                            Ok(content) => {
                                                 ...
                                                 println!("?諭?[Analyzer] Ingesting file update: {}", path);
                                                 if let Err(e) = mem.add(&content, meta).await {
                                                     eprintln!("?醫묓닔 [Analyzer] Mem Ingest Failed: {}", e);
                                                 }
                                            },
                                            Err(e) => eprintln!("?醫묓닔 [Analyzer] Failed to read file {}: {}", path, e),
                                        }
                                    }
                                }
                            }
                    ...
                    if let Err(e) = db::insert_event_v2(&masked_event) {
                        eprintln!("?醫묓닔 [Analyzer] DB Insert Error: {}", e);
                    }
                 ...
                 eprintln!("?醫묓닔 [Analyzer] Failed to parse log as EventEnvelope: {}", log_json);
            }
        }
    });
}
...
async fn process_buffer(
    ...
    for session in &sessions {
        if let Err(e) = db::insert_session(session) {
            eprintln!("?醫묓닔 [Analyzer] Failed to save session: {}", e);
        }
        ...
        if let Some(mem) = memory {
            ...
            if let Err(e) = mem.add(&summary, meta).await {
                eprintln!("?醫묓닔 [Memory] Ingestion Failed: {}", e);
            }
        }
    }
    ...
    if !patterns.is_empty() {
        println!("?彛?[Analyzer] Detected {} patterns in recent activity.", patterns.len());
    }
    ...
    for pattern in patterns {
        ...
        if let Err(e) = db::insert_routine_candidate(&pattern) {
             eprintln!("?醫묓닔 [Analyzer] Failed to save routine candidate: {}", e);
        }
        ...
        if let Some(proposal) = matcher.match_pattern(&pattern) {
            if proposal.confidence >= min_confidence {
                ...
                if let Err(e) = db::insert_recommendation(&proposal) {
                     eprintln!("?醫묓닔 [Analyzer] DB Error: {}", e);
                } else {
                     let _ = notifier::send("Steer OS", &format!("?裕?New Idea: {}", proposal.title));
                     remaining_budget -= 1;
                }
        ...
        println!("?夷?[Analyzer] Hybrid Intelligence: Routing to {} (Model: {})", 
                ...
                if proposal.confidence >= min_confidence {
                    ...
                    if let Err(e) = db::insert_recommendation(&proposal) {
                        eprintln!("?醫묓닔 [Analyzer] DB Error: {}", e);
                         ...
                         let _ = notifier::send("Steer OS", &format!("??New Idea (AI): {}", proposal.title));
                         remaining_budget -= 1;
                    }
                }
            },
            Err(e) => {
                eprintln!("?醫묓닔 [Analyzer] AI Recommendation Failed: {}", e);
            }
        }
    }
