...
pub enum PatternType {
    AppSequence,      // ???袁れ넎 ??쀂???(?? Chrome ??Slack ??Gmail)
    KeywordRepeat,    // 獄쏆꼶????쇱뜖????낆젾
    FilePattern,      // ???뵬 ?臾믩씜 ???쉘
    TimeBasedAction,  // ??볦퍢 疫꿸퀡而?獄쏆꼶???臾믩씜
}
...
pub struct PatternConfig {
    pub min_occurrences: u32,       // 筌ㅼ뮇??獄쏆꼶????쏅땾 (疫꿸퀡?? 3)
    pub min_similarity: f64,         // 筌ㅼ뮇???醫롪텢??(疫꿸퀡?? 0.8)
    pub lookback_days: i64,          // ?브쑴苑?疫꿸퀗而?(疫꿸퀡?? 7??
}
