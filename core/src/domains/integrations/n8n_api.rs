...
impl N8nApi {
    ...
    pub fn from_env() -> anyhow::Result<Self> {
        let base_url = std::env::var("N8N_API_URL").unwrap_or_else(|_| "http://localhost:5678/api/v1".to_string());
    ...
    pub async fn ensure_server_running(&self) -> Result<()> {
        ...
        let health_url = self.base_url.replace("localhost", "127.0.0.1").replace("/api/v1", "/"); 
        
        println!("?逾?Checking n8n health at {}...", health_url);

        // 1. Check if running
        if self.client.get(&health_url).timeout(std::time::Duration::from_secs(2)).send().await.is_ok() {
            println!("??n8n server is running.");
            // self.verify_auth().await?; // Disabled to prevent 503 if user hasn't set up API key yet
            return Ok(());
        }

        println!("?醫묓닔  n8n server NOT found. Starting automatically...");
        ...
        let n8n_command = "n8n.cmd";

        #[cfg(not(target_os = "windows"))]
        let n8n_command = "npx";

        #[cfg(not(target_os = "windows"))]
        let n8n_args = vec!["-y", "n8n", "start"];

        #[cfg(target_os = "windows")]
        let n8n_args = vec!["start"];

        let _child = Command::new(n8n_command)
            .args(&n8n_args)
        ...
        for i in 0..30 {
            ...
            if i % 5 == 0 { println!("... still waiting ({}/60s)", i * 2); }
        }
    ...
    pub async fn verify_auth(&self) -> Result<()> {
        println!("?逾?Verifying n8n API Key...");
        ...
        if resp.status().is_success() {
             ...
             Err(anyhow::anyhow!("??n8n API Key is INVALID (401). Check core/.env or secrets."))
        } else {
             ...
             println!("?醫묓닔 Auth check returned status: {}", resp.status());
    ...
    pub async fn list_credentials(&self) -> Result<Vec<Credential>> {
        ...
        let data = json["data"].as_array().ok_or_else(|| anyhow::anyhow!("Invalid credentials response"))?;
        
        // n8n API structure differs by version, trying to extract minimal info
        let credentials = data.iter().map(|c| Credential {
    ...
    pub async fn create_workflow(&self, name: &str, workflow_json: &Value, active: bool) -> Result<String> {
        ...
        let is_empty = normalized.get("nodes")
            .and_then(|n| n.as_array())
            .map(|arr| arr.is_empty())
            .unwrap_or(true);

        if is_empty {
            println!("?醫묓닔 Workflow nodes empty. Falling back to minimal workflow template.");
        ...
        if !self.api_key.is_empty() && self.api_key != "placeholder" {
            ...
            match self.create_workflow_api(name, &normalized, active).await {
                Ok(id) => return Ok(id),
                Err(e) => println!("?醫묓닔 API creation failed ({}). Falling back to CLI...", e),
            }
        } else {
            println!("?諭꾪닔 No API Key configured. Using CLI mode.");
        }
    ...
    async fn create_workflow_cli(&self, name: &str, workflow_json: &Value, active: bool) -> Result<String> {
        // Prepare JSON file
        ...
        println!("?諭?Importing workflow via CLI from {}...", path);
        ...
        if let Err(e) = tokio::fs::remove_file(&path).await {
             eprintln!("?醫묓닔 Failed to clean up temp file: {}", e);
        }
        ...
        if !output.status.success() {
             ...
             return Err(anyhow::anyhow!("CLI Import failed (exit {}): {}", code, detail));
        }

        println!("??CLI Import successful! Now fixing ownership...");
        
        if let Err(e) = self.fix_workflow_ownership() {
            println!("?醫묓닔 Ownership fix failed: {}", e);
        }
    ...
    fn fix_workflow_ownership(&self) -> Result<()> {
        ...
        if !project_id.is_empty() {
            println!("?逾?Linking workflows to Project ID: {}", project_id);
            ...
            let orphan_ids: Vec<String> = stmt.query_map([], |row| row.get(0))?
    ...
    pub async fn list_workflows(&self) -> Result<Vec<WorkflowStatus>> {
        ...
        let data: Value = resp.json().await?;
        let workflows = data["data"].as_array()
            .map(|arr| {
                arr.iter().map(|w| WorkflowStatus {
    ...
    pub async fn list_executions(&self, workflow_id: &str, limit: u32) -> Result<Vec<ExecutionResult>> {
        let url = format!("{}/executions?workflowId={}&limit={}", self.base_url, workflow_id, limit);
        ...
        let data: Value = resp.json().await?;
        let executions = data["data"].as_array()
            .map(|arr| {
                arr.iter().map(|e| ExecutionResult {
