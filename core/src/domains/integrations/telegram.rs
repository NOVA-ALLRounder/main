...
impl TelegramBot {
    ...
    pub async fn start_polling(self: Arc<Self>) {
        info!("?夷?Telegram Bot started. Waiting for messages...");
        let mut offset = 0;
        ...
        loop {
            match self.get_updates(offset).await {
                        ...
                        if let Some(msg) = update.message {
                            if let Some(text) = msg.text {
                                if self.is_allowed(&msg.from) {
                                    info!("?踰?Received command: '{}'", text);
                                    let bot_clone = self.clone();
                                    let chat_id = msg.chat.id;
                                    let text_clone = text.clone();
                                    
                                    // Ack reception
                                    let _ = self.send_message(chat_id, "?夷?Command received. Processing...").await;
                ...
                Err(e) => {
                    error!("?醫묓닔 Telegram Poll Error: {}", e);
