...
impl DynamicController {
    ...
    pub async fn surf(&self, goal: &str) -> Result<()> {
        ...
        for i in 1..=self.max_steps {
            info!("\n?遊?[Step {}/{}] Observing...", i, self.max_steps);
            ...
            match action_type {
                ...
                "save_routine" => {
                    ...
                    info!("      ?裕?Saving Routine as '{}' ({} steps)...", name, session_steps.len());
                    ...
                    match db::get_learned_routine(name) {
                        Ok(Some(routine)) => {
                            match serde_json::from_str::<Vec<SmartStep>>(&routine.steps_json) {
                                Ok(steps) => {
                                    info!("      ??고닔 Loaded {} steps. Executing...", steps.len());
                ...
                _ => {
                    warn!("?醫묓닔 Unknown action: {}", action_type);
                }
            }
            ...
            if consecutive_failures >= 2 {
                println!("      ?酉귦닔 CRITICAL: Consecutive failures detected ({})", consecutive_failures);
                println!("      ??녔닼?ENGAGING COMBAT PROTOCOL: Attempting to clear obstacles...");
                
                // Strategy 1: The "Escape" Hatch (Close Modals)
                println!("         ?紐?Strategy 1: Pressing ESC");
                ...
                println!("         ?紐?Strategy 2: Pressing ENTER");
            ...
            println!("      ?紐볠닼?Monitoring UI stability...");
            if let Err(e) = VisualDriver::wait_for_ui_settle(3000).await {
                println!("      ?醫묓닔 Adaptive wait error: {}", e);
            }
