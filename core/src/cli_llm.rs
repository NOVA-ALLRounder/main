...
use log::{debug, warn}; // Added missing imports
...
impl CLILLMClient {
    ...
    pub fn execute(&self, prompt: &str) -> Result<String> {
        ...
        let use_stdin = match self.provider {
            LLMProvider::Codex => {
                cmd.args(&["exec", "--sandbox", "danger-full-access", "--skip-git-repo-check", "--color", "never", "-"]);
                true
            },
            LLMProvider::Gemini => {
        ...
        match Self::extract_json(&raw_output) {
            ...
            None => {
                ...
                Err(anyhow::anyhow!("No valid JSON found in CLI output: {}", raw_output))
            }
        }
    }
...
pub fn execute_cli_llm(prompt: &str) -> Result<String> {
    let client = CLILLMClient::from_env()
        .ok_or_else(|| anyhow!("STEER_CLI_LLM not set. Use: export STEER_CLI_LLM=gemini|codex|claude"))?;
