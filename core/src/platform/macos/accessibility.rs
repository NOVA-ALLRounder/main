...
/// macOS Accessibility Provider implementation
...
impl AccessibilityProvider for MacOSAccessibility {
    ...
    fn get_selected_text(&self) -> Result<Option<String>> {
        Ok(get_selected_text())
    }

    fn element_exists_at(&self, x: i32, y: i32) -> Result<bool> {
        Ok(get_element_center_at(x, y).is_some())
    }
    ...
    fn get_element_info(&self, x: i32, y: i32) -> Result<Option<Value>> {
        unsafe {
            ...
            let role = get_string_attribute(element_ref, "AXRole").unwrap_or_default();
            let title = get_string_attribute(element_ref, "AXTitle").unwrap_or_default();
            let value = get_string_attribute(element_ref, "AXValue").unwrap_or_default();
