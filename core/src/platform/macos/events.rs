            ...
            move |_proxy, type_, event| {
                ...
                if let Ok(log) = serde_json::to_string(&log_json) {
                    if let Err(e) = tx.try_send(log) {
                        ...
                        match e {
                            TrySendError::Full(_) => eprintln!("?醫묓닔 [MacOS] Event Channel Full! Dropping event (increase buffer?)."),
                            TrySendError::Closed(_) => eprintln!("?醫묓닔 [MacOS] Event Channel Closed."),
                        }
                    }
                } else {
                    eprintln!("?醫묓닔 [MacOS] Failed to serialize event; dropping.");
                }
...
/// macOS Event Monitor implementation
...
impl EventMonitor for MacOSEventMonitor {
    ...
    fn stop(&self) -> Result<()> {
        // Note: Currently no way to stop the event tap cleanly
        // This would require storing the tap handle and calling CFRunLoopStop
        Ok(())
    }
}
