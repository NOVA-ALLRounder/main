...
def _build_payload(
    ...
    *,
    ...
    # ── actions 데이터에서 구체적 행동 단계 추출 ──
    action_summary = daily.get("action_summary") or {}
    action_steps = _extract_action_steps(action_summary, include_apps)
    ...
    return {
        ...
        "notes": [
            ...
            "action_steps show concrete user actions (clicks, inputs, shortcuts) per app.",
        ],
    }
...
def _compress_payload(payload: dict, *, max_bytes: int) -> dict:
    ...
    # action_steps 축소: 시퀀스 수 제한
    action_steps = compact.get("action_steps") or {}
    if action_steps.get("repeated_sequences"):
        action_steps["repeated_sequences"] = action_steps["repeated_sequences"][:3]
    if action_steps.get("app_profiles"):
        action_steps["app_profiles"] = action_steps["app_profiles"][:5]
    compact["action_steps"] = action_steps
    if _size(compact) <= max_bytes:
        return compact
    ...
    compact["action_steps"] = {}
    compact["notes"] = ["compressed: reduced lists for size limit"]
    return compact


def _extract_action_steps(action_summary: dict, include_apps: set[str]) -> dict:
    """action_summary에서 LLM이 이해할 수 있는 구체적 행동 단계를 추출."""
    if not action_summary or action_summary.get("total_actions", 0) == 0:
        return {}

    # 앱별 행동 프로필 (자주 하는 행동)
    app_profiles = action_summary.get("app_action_profiles") or []
    if include_apps:
        app_profiles = [
            p for p in app_profiles
            if p.get("app", "").upper() in {a.upper() for a in include_apps}
        ]
    # 상위 5개 앱만
    app_profiles = app_profiles[:5]

    # 반복되는 시퀀스 (같은 순서의 행동이 여러 번 반복된 것)
    sequences = action_summary.get("top_action_sequences") or []
    if include_apps:
        sequences = [
            s for s in sequences
            if s.get("app", "").upper() in {a.upper() for a in include_apps}
        ]
    sequences = sequences[:5]

    result = {
        "total_actions": action_summary.get("total_actions", 0),
        "action_types": action_summary.get("by_type", {}),
    }
        ...
        result["app_profiles"] = [
            {
                ...
                "top_actions": p.get("top_actions", [])[:5],
            }
