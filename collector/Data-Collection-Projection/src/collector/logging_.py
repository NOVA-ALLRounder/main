    ...
    *,
    ...
    if log_dir:
        ...
        if activity_detail_text_file:
            ...
            activity_text_logger.propagate = False
            ...
            # ── 활동 로그 전용 콘솔 출력 (보기 쉬운 포맷) ──
            if activity_to_console:
                activity_console = logging.StreamHandler()
                activity_console.setFormatter(
                    ActivityConsoleFormatter(tzinfo=tzinfo)
                )
                activity_text_logger.addHandler(activity_console)
        else:
            # text_file이 설정되지 않은 경우에도 콘솔 출력은 지원
            if activity_to_console:
                activity_text_logger = logging.getLogger("collector.activity_text")
                activity_text_logger.setLevel(root.level)
                activity_text_logger.propagate = False
    ...
    if to_console or not log_dir:
        ...
        # activity_text 로거 메시지가 콘솔에 중복 출력되지 않도록 필터
        console_handler.addFilter(_ExcludeActivityTextFilter())
...
class ActivityConsoleFormatter(logging.Formatter):
    """활동 로그 전용 콘솔 포맷터 - [HH:MM:SS] 메시지 형태로 깔끔하게 출력."""
    ...
    def format(self, record: logging.LogRecord) -> str:
        ts = _format_ts(record.created, self._tzinfo)
        # 시간 부분만 추출 (YYYY-MM-DD HH:MM:SS → HH:MM:SS)
        time_part = ts.split(" ", 1)[-1] if " " in ts else ts
        message = record.getMessage()
        return f"[{time_part}] {message}"


class _ExcludeActivityTextFilter(logging.Filter):
    """activity_text 로거의 메시지를 루트 콘솔에서 제외."""

    def filter(self, record: logging.LogRecord) -> bool:
        return not record.name.startswith("collector.activity_text")
