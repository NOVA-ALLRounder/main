...
def run() -> None:
    ...
    action_buffer = None
    action_collector_inst = None
    pattern_analyzer_inst = None
    ...
    try:
        ...
        action_collector_inst = ActionCollector(store)
        pattern_analyzer_inst = PatternAnalyzer(store)

        def _on_action_complete(action):
            action_collector_inst.on_action(action)
            pattern_analyzer_inst.on_action(action)
        ...
        action_buffer.start()
        logger.info("action-level processing enabled")
    except Exception:
        logger.info("action-level processing disabled (import error)")
    ...
    bus = EventBus(
        ...
        action_collector=action_collector_inst,
        pattern_analyzer=pattern_analyzer_inst,
    )
    bus.start()
