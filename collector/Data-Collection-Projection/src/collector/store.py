...
class SQLiteStore:
    ...
    def insert_action(self, action) -> bool:
        """ActionBuffer에서 생성된 Action을 actions 테이블에 저장."""
        if self._conn is None:
            raise RuntimeError("database is not connected")
        try:
            with self._lock:
                self._conn.execute(
                    ...
                    INSERT OR IGNORE INTO actions (
                        action_id, action_type, app, window_title,
                        control_name, control_type, final_value,
    ...
    def insert_pattern(self, pattern: dict) -> bool:
        """PatternAnalyzer에서 감지된 패턴을 potential_patterns 테이블에 저장."""
        if self._conn is None:
            raise RuntimeError("database is not connected")
        try:
            with self._lock:
                self._conn.execute(
                    """
                    INSERT OR REPLACE INTO potential_patterns (
                        pattern_hash, sequence, occurrence_count,
                        pattern_length, detected_at, actions_json
                    )
                    VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    (
    ...
    def fetch_actions(
        ...
        if self._conn is None:
            raise RuntimeError("database is not connected")
        query = (
            "SELECT action_id, action_type, app, window_title, control_name, "
            "control_type, final_value, started_at, ended_at, duration_ms, "
            "metadata_json FROM actions"
        )
        params: list = []
        clauses: list[str] = []
        if start_ts:
