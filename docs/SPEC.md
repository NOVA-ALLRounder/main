...
## 1. Project Overview
사용자의 로컬 OS(Windows)를 대상으로 자연어 명령을 해석하여 실제 실행(Execution), 검증(Verification), 복구(Recovery)를 수행하는 전역 권한 에이전트.
...
* **OS Dependence (Action):** 실제 제어는 Native API(Windows UI Automation/Win32) 또는 보조 도구 사용.
* **User Control:** 사용자는 언제든 에이전트의 동작을 강제 중단할 수 있어야 함.

## 4. MVP Roadmap
* **Phase 1:** Read-only Agent (Screen Capture, UIA Tree 조회) + Basic Mouse Move.
