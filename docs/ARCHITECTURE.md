...
## 1. High-Level Design
`[LLM] <-> [Rust Core (Broker)] <-> [Windows Adapter] <-> [Win32/UI Automation APIs]`

## 2. Component Responsibility

### A. Rust Core (The Brain)
* **Role:** 상태 관리, 보안 정책 집행, LLM 컨텍스트 관리.
* **Reasoning:** Windows Adapter는 로직을 갖지 않으며, 오직 Rust의 명령만 수행.
* **State Machine:**
...
### B. Windows Adapter (The Hands)
* **Role:** Windows Native API 래핑 및 실행.
* **Scope:**
  * `UI Automation (UIA)`: UI 요소 탐색 및 제어.
  * `SendInput`: 마우스/키보드 이벤트 시뮬레이션.
  * `Desktop Duplication / GDI`: 스크린 캡처.
