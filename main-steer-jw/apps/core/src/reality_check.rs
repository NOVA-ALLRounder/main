        ...
        if let Some(ref apps) = INSTALLED_APPS {
            ...
            for installed in apps {
                if installed.contains(&target) || target.contains(installed) {
                     println!("      ⚠️ [Reality] Fuzzy match: '{}' -> '{}'", app_name, installed);
                     return Ok(installed.clone()); // Return the actual installed name
                }
            }
            
            println!("      ❌ [Reality] REJECTED: App '{}' is not installed.", app_name);
            return Err(anyhow!("HALLUCINATION DETECTED: Application '{}' is not installed on this machine.", app_name));
        } else {
...
pub fn verify_file_exists(path: &str) -> Result<()> {
    ...
    if path_obj.exists() {
        ...
        Err(anyhow!("HALLUCINATION DETECTED: File '{}' does not exist.", path))
    }
}
