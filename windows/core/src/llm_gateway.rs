...
static EMBEDDING_FALLBACK_WARNED: AtomicBool = AtomicBool::new(false);
...
impl OpenAILLMClient {
    ...
    fn embedding_fallback_enabled() -> bool {
        env::var("STEER_EMBEDDING_FALLBACK")
            .ok()
            .map(|v| {
                matches!(
    ...
    fn is_quota_or_rate_error(error_text: &str) -> bool {
    ...
    fn warn_embedding_fallback_once(reason: &str) {
        if EMBEDDING_FALLBACK_WARNED
            .compare_exchange(false, true, Ordering::SeqCst, Ordering::SeqCst)
            .is_ok()
        {
            eprintln!(
                "⚠️ [Embedding] Cloud embedding unavailable. Using deterministic local fallback (reason: {}).",
                reason
            );
        }
    }
    ...
    fn deterministic_embedding(text: &str, dim: usize) -> Vec<f32> {
        ...
        for token in text.split_whitespace() {
            ...
            let magnitude = (digest[5] as f32 / 255.0) + 0.25;
            vec[idx] += sign * magnitude;

            let secondary_idx = u32::from_le_bytes([digest[6], digest[7], digest[8], digest[9]])
                as usize
                % dim;
            let secondary_sign = if digest[10] % 2 == 0 { 1.0 } else { -1.0 };
            vec[secondary_idx] += secondary_sign * 0.35;
        }
    ...
    fn heuristic_recommendation_from_pattern(
        pattern_description: &str,
        sample_events: &[String],
    ) -> AutomationProposal {
        let compact_pattern = pattern_description
            .split_whitespace()
            .collect::<Vec<_>>()
            .join(" ");
        let title_seed = compact_pattern.chars().take(42).collect::<String>();
        let title = if title_seed.is_empty() {
            "반복 작업 자동화 제안".to_string()
        } else {
            format!("반복 작업 자동화: {}", title_seed)
        };

        let trigger_seed = sample_events
            .first()
            .cloned()
            .unwrap_or_else(|| "pattern_detected".to_string());
        let trigger = trigger_seed.chars().take(96).collect::<String>();

        AutomationProposal {
            title,
            summary: "클라우드 LLM 호출이 불가하여 휴리스틱 기반 제안을 생성했습니다."
                .to_string(),
            ...
            actions: vec![
                "반복 패턴 감지".to_string(),
                "조건 확인".to_string(),
                "자동 알림 또는 실행".to_string(),
            ],
            n8n_prompt: format!(
                "Detect this recurring pattern and automate follow-up actions: {}",
                compact_pattern
            ),
            confidence: 0.42,
            evidence: vec![],
    ...
    fn get_workflow_context(&self) -> String {
        ...
        if cfg!(target_os = "windows") {
            context.push_str("- Mouse: prefer Win32/UIA integrations first.\n");
            context.push_str("- Keyboard: use PowerShell (`WScript.Shell.SendKeys`) for fallback automation.\n");
            context.push_str("- Avoid AppleScript/osascript/cliclick in Windows workflows.\n\n");
        } else {
            ...
            if has_cliclick {
                ...
                context.push_str(
                    "  - Use: `cliclick c:x,y` (click), `cliclick dc:x,y` (double click)\n",
                );
            } else {
                context.push_str("- Mouse control: 'cliclick' is not installed.\n");
                context.push_str("  - Preferred: use AppleScript via `osascript` for basic clicks or suggest installing cliclick.\n");
                context.push_str("  - Command: `osascript -e 'tell application \"System Events\" to click at {x,y}'`\n");
            }
...
impl LLMClient for OpenAILLMClient {
    ...
    async fn plan_next_step(
        ...
        let system_prompt = r#"
You are a Desktop Automation Agent. Your job is to FULLY achieve the user's goal.
...
"#;
            ...
            .await
        {
            ...
            Err(e) => {
                eprintln!(
                    "⚠️ [plan_next_step] OpenAI request failed: {}. Invoking fallback chain...",
                    e
                );
                let messages = vec![
                    json!({"role": "system", "content": system_prompt}),
    ...
    async fn chat_completion(&self, messages: Vec<Value>) -> Result<String> {
        ...
        let res_json: serde_json::Value = match self
            ...
            .await
        {
            Ok(res) => {
                if res.status().is_success() {
                    ...
                    let fallback = crate::cli_llm::fallback_chat_completion(&messages).await?;
                    return Ok(fallback);
                }
            }
            Err(e) => {
                eprintln!(
                    "⚠️ [chat_completion] OpenAI request failed: {}. Invoking fallback chain...",
                    e
                );
                let fallback = crate::cli_llm::fallback_chat_completion(&messages).await?;
                return Ok(fallback);
    ...
    async fn plan_vision_step(
        ...
        // CLI-first mode: skip OpenAI primary and run the CLI provider chain first.
        if Self::cli_first_enabled() {
...
"#,
            ...
            .await
        {
            Ok(res) => {
                if res.status().is_success() {
                    ...
                    let error_text = res.text().await.unwrap_or_default();
                    eprintln!(
                        "⚠️ [plan_vision_step] OpenAI error/rate-limited: {}. Invoking fallback chain (text-only)...",
                        error_text
                    );
            ...
            Err(e) => {
                eprintln!(
                    "⚠️ [plan_vision_step] OpenAI request failed: {}. Invoking fallback chain (text-only)...",
                    e
                );
                let text_messages = vec![
                    json!({"role": "system", "content": system_prompt}),
                ...
                let fallback_result = crate::cli_llm::fallback_chat_completion(&text_messages).await?;
                let action_json = match recover_json(&fallback_result) {
                    Some(v) => v,
                    None => {
                        let fallback_action = self.fallback_vision_action(goal, history);
    ...
    async fn recommend_automation(&self, logs: &[String]) -> Result<String> {
        ...
        let (script_type, script_fence, script_header) = if cfg!(target_os = "windows") {
            ("POWERSHELL SCRIPT (or Python)", "powershell", "# PowerShell")
        } else {
            ("BASH SCRIPT (or Python)", "bash", "#!/bin/bash")
        };

        let prompt = format!(
            "Based on the user behavior logs (JSON) below, identify a repetitive manual task that can be automated.\n\
            Then, generate a robust {} to automate it.\n\
            \n\
            Output Format:\n\
            ### Problem\n\
            (Description)\n\
...
"##;
            ...
            .await
        {
            ...
            Err(e) => {
                eprintln!(
                    "⚠️ [build_n8n_workflow] OpenAI request failed: {}. Invoking fallback...",
                    e
                );
                let messages = vec![
                    json!({"role": "system", "content": system_prompt}),
...
"#;
            ...
            .await
        {
            ...
            Err(e) => {
                eprintln!(
                    "⚠️ [propose_workflow] OpenAI request failed: {}. Invoking fallback...",
                    e
                );
                let messages = vec![
                    json!({"role": "system", "content": system_prompt}),
...
"#;
            ...
            .await
        {
            ...
            Err(e) => {
                eprintln!(
                    "⚠️ [parse_intent] OpenAI request failed: {}. Invoking fallback...",
                    e
                );
                let fallback_res = crate::cli_llm::fallback_chat_completion(&messages).await?;
                let parsed = recover_json(&fallback_res)
    ...
    async fn get_embedding(&self, text: &str) -> Result<Vec<f32>> {
            ...
            .await
        {
            ...
            Err(err) => {
                if Self::embedding_fallback_enabled() {
                    Self::warn_embedding_fallback_once(&err.to_string());
                    return Ok(Self::deterministic_embedding(text, 1536));
                }
                return Err(err);
            }
        };
        ...
        if !response.status().is_success() {
            let error_text = response.text().await.unwrap_or_default();
            if Self::embedding_fallback_enabled() && Self::is_quota_or_rate_error(&error_text) {
                Self::warn_embedding_fallback_once(&error_text);
                return Ok(Self::deterministic_embedding(text, 1536));
            }
            ...
            .await
        {
            Ok(response) => {
                if response.status().is_success() {
                    ...
                    let fallback_res = match crate::cli_llm::fallback_chat_completion(&messages).await
                    {
                        Ok(raw) => raw,
                        Err(fallback_err) => {
                            if Self::is_quota_or_rate_error(&error_text) {
                                eprintln!(
                                    "⚠️ [generate_recommendation] CLI fallback unavailable after quota/rate issue ({}). Using heuristic fallback.",
                                    fallback_err
                                );
                                return Ok(Self::heuristic_recommendation_from_pattern(
                                    pattern_description,
                                    sample_events,
                                ));
                            }
                            return Err(anyhow::anyhow!(
                                "OpenAI recommendation error: {} | CLI fallback failed: {}",
                                error_text,
                                fallback_err
                            ));
                        }
                    };
                    let parsed: Value = match recover_json(&fallback_res) {
                        Some(v) => v,
                        None => {
                            if Self::is_quota_or_rate_error(&error_text) {
                                return Ok(Self::heuristic_recommendation_from_pattern(
                            ...
                            return Err(anyhow::anyhow!(
                                "Failed to recover JSON from fallback"
                            ));
                        }
                    };
                    json!({
            ...
            Err(e) => {
                eprintln!(
                    "⚠️ [generate_recommendation] OpenAI request failed: {}. Invoking fallback...",
                    e
                );
                let fallback_res = match crate::cli_llm::fallback_chat_completion(&messages).await {
                    Ok(raw) => raw,
                    Err(fallback_err) => {
                        let upstream = e.to_string();
                        if Self::is_quota_or_rate_error(&upstream) {
                            eprintln!(
                                "⚠️ [generate_recommendation] CLI fallback unavailable after quota/rate request failure ({}). Using heuristic fallback.",
                                fallback_err
                            );
                            return Ok(Self::heuristic_recommendation_from_pattern(
                                pattern_description,
                        ...
                        return Err(anyhow::anyhow!(
                            "OpenAI request failed: {} | CLI fallback failed: {}",
                            upstream,
                            fallback_err
                        ));
                    }
