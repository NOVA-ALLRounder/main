...
use chrono::Utc;
use local_os_agent::schema::AgentAction;
#[cfg(target_os = "macos")]
...
fn default_downloads_dir() -> String {
    #[cfg(target_os = "windows")]
    {
        ...
        if let Ok(userprofile) = std::env::var("USERPROFILE") {
            let base = userprofile.trim().trim_end_matches('\\').trim_end_matches('/');
            if !base.is_empty() {
                return format!("{}\\Downloads", base);
            }
        }
        return "C:\\Users\\Public\\Downloads".to_string();
    }
...
async fn main() -> anyhow::Result<()> {
    ...
    let ax_check = tokio::time::timeout(std::time::Duration::from_secs(3), async {
        local_os_agent::applescript::check_accessibility()
    })
    .await;
    ...
    loop {
        ...
        match parts[0] {
            ...
            "snap" => {
                ...
                {
                    println!("Snapshot command is only available on macOS accessibility mode.");
                }
            }
            "type" => {
                if parts.len() < 2 {
