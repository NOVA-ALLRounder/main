...
impl ActionRunner {
    ...
    async fn recover_focus_and_verify(
        ...
        let bridge_app = if cfg!(target_os = "windows") {
            "Explorer"
        } else {
            "Finder"
        };
        ...
        while !applescript::app_names_match(&front, target_app) && attempts < retries {
            attempts += 1;
        ...
        if !applescript::app_names_match(&front, target_app)
            && Self::focus_recovery_profile() == "aggressive"
        {
            if heuristics::try_close_front_dialog() {
                recovery_trace.push("dialog_closed".to_string());
            }
            let _ = heuristics::ensure_app_focus(bridge_app, 2).await;
            attempts += 1;
        ...
        (
            applescript::app_names_match(&front, target_app),
            front,
            attempts,
            recovery_trace,
        )
        ...
        match action_type {
            ...
            "click_ref" => {
                ...
                if ref_id.is_empty() {
                    ...
                    let plan_targets_finder = plan["app"]
                        .as_str()
                        .map(|v| applescript::app_names_match(v, "Finder"))
                        .unwrap_or(false)
                        || plan["name"]
                            .as_str()
                            .map(|v| applescript::app_names_match(v, "Finder"))
                    ...
                    if finder_download_target
                        && (applescript::app_names_match(&front_app, "Finder")
                            || plan_targets_finder)
                    {
            ...
            "click_visual" => {
                ...
                } else if applescript::app_names_match(&front_app, "Finder")
                    && Self::goal_mentions_downloads(goal)
