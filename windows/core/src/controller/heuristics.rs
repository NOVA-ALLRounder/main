...
pub fn permission_help() -> &'static str {
    ...
    {
        "Allow screen capture and UI automation for this app (run from an interactive desktop session, and if needed restart elevated)."
    }
...
pub fn preflight_permissions() -> Result<()> {
    ...
    #[cfg(target_os = "windows")]
    {
        // Windows does not require macOS Accessibility entitlement checks.
        // Keep a lightweight runtime probe; non-interactive contexts may return empty.
        let _ = crate::tool_chaining::CrossAppBridge::get_frontmost_app();
    }
...
pub fn focus_text_area(app: &str, prefer_subject: bool) -> bool {
    #[cfg(target_os = "windows")]
    {
        let candidates: &[&str] = match app {
            "Mail" if prefer_subject => &["Subject", "Add a subject", "제목"],
            "Mail" => &["Message", "Body", "본문"],
            "Notes" => &["Text", "Editor", "본문"],
            "TextEdit" => &["Text", "Editor", "Document"],
            _ => &[],
        };

        for name in candidates {
            if crate::macos::actions::click_element(name).is_ok() {
                return true;
            }
        }

        // Fallback: nudge focus with TAB to enter editable region.
        let tab_sequence = if app.eq_ignore_ascii_case("Mail") && prefer_subject {
            "{TAB}{TAB}"
        } else {
            "{TAB}"
        };
...
pub fn frontmost_browser(front_app: Option<&str>) -> Option<&'static str> {
    ...
    {
        match front_app {
            Some(app) if applescript::app_names_match(app, "Google Chrome") => Some("Google Chrome"),
            Some(app)
                if applescript::app_names_match(app, "Microsoft Edge")
                    || applescript::app_names_match(app, "Safari") =>
            {
                Some("Microsoft Edge")
            }
            _ => None,
    ...
    {
    match front_app {
        Some(app) if applescript::app_names_match(app, "Safari") => Some("Safari"),
        Some(app) if applescript::app_names_match(app, "Google Chrome") => Some("Google Chrome"),
        _ => None,
    }
    }
...
pub fn try_close_front_dialog() -> bool {
    #[cfg(target_os = "windows")]
    {
        for label in ["Cancel", "취소", "Close", "닫기"] {
            if crate::win32_app_control::system_events::click_button_by_name(label).is_ok() {
                return true;
            }
        }

        // Final fallback: ESC key to dismiss active modal.
...
pub async fn ensure_app_focus(target_app: &str, retries: usize) -> bool {
    ...
    if let Ok(front) = crate::tool_chaining::CrossAppBridge::get_frontmost_app() {
        if applescript::app_names_match(&front, target_app) {
            return true;
        }
    }
