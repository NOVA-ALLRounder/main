...
def _ensure_core_binary() -> str:
    exe_name = "local_os_agent.exe" if os.name == "nt" else "local_os_agent"
    binary = Path("./core/target/debug") / exe_name
...
def test_integration():
    ...
    try:
        ...
        preflight_raw = _http_get(f"http://127.0.0.1:{port}/api/agent/preflight")
        ...
        req = urllib.request.Request(
            f"http://127.0.0.1:{port}/api/agent/preflight/fix",
            data=json.dumps({"action": "activate_finder"}).encode("utf-8"),
            headers={"Content-Type": "application/json"},
            method="POST",
        )
        with urllib.request.urlopen(req, timeout=3) as resp:
            fix_raw = resp.read().decode("utf-8")
        fix = json.loads(fix_raw)
        assert "ok" in fix
        assert fix.get("action") == "activate_finder"
    finally:
